 ...
 * @since  1.6
 */
class TgripartiModelNominativoForm extends JModelForm
{
	private $item = null;
	...
	{
		// Get the form.
		$form = $this->loadForm('com_tgriparti.nominativo', 'nominativoform', array(
			'control'   => 'jform',
			'load_data' => $loadData
			)
		);
	...
	{
		$data = JFactory::getApplication()->getUserState('com_tgriparti.edit.nominativo.data', array());
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('nominativo.id');
